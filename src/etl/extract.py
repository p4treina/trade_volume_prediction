...
class DataExtractor:
    ...
    def rename_columns(self, df: pd.DataFrame, target) -> pd.DataFrame:
        ...
        new_columns = ["date"] + [
            column + f"_{target}" for column in df.columns if column != date_col
        ]
    ...
    @staticmethod
    def remove_constant_period(
        df: pd.DataFrame, date_begin: str = "4/1/2020", date_end: str = "5/29/2020"
    ) -> pd.DataFrame:
    ...
    def extract_data(self, target: str = "gm") -> pd.DataFrame:
        df = self.load_data()
        df = self.rename_columns(df, target)
